         ...
         * --- Solicita os Itens
         ...
         Begin Sequence
            ...
            * --- Salva Area da Janela
            ...
            Cor_Ant   := setcolor()
            Sombra(06,10,17,72)
            Window( 06, 10, 17, 72, [ (T R O C O)]) //, 5, [ þ Esc - Retorna], C_CDTit, C_CDFnd )
            @ 15,14 say "Total:"
...
Function Desconto(PPer,Pdes,Pval)
   ...
   Sombra(13,24,19,72)
   Caixa_Smp( 13, 24, 19, 72, [ D E S C O N T O ], 5, [ þ Esc - Retorna], C_CDTit, C_CDFnd )
...
Function SubTot()
   ...
   Sombra(03,01,15,78)
   cCorAntes := setcolor()
   Window( 03, 01, 15, 78, " P A G A M E N T O S "," þ Esc-Retorna | Escolha a op‡„o (1,2,3,4,5,6,7,9) ")
...
Function CancelaItem()
   ...
   If MSubTot = 0
	...
	Set Key K_F5 To
	AEVal(VCodPro,{|elem| ++nI,aadd(aitem,[ ]+str(nI,3)+[ ]+pad(elem,13)+[  ]+if(empty(VDesPro[nI]),"*ÄÄÄÄÄÄÄÄÄÄ C A N C E L A D O ÄÄÄÄÄÄÄÄ-*",Pad(VDesPro[nI],34))+space(4)+transform(VQtdPro[nI],[999,999])+[ ]+transform(VPcoPro[nI]*VQtdPro[nI],[@e 999,999.99])+[ ]),aadd(aSelec,!empty(VDesPro[nI]))})  
...
procedure GravaCupom
   ...
   if !VendCup->(dbsetorder(1),dbseek(cNumPdv+cNumCupom))
      ...
      enddo
      VendCup->LancNfce := cLancNfce  // ** Numero do lançamento
...
procedure Tela
   ...
   //                    1         2         3         4         5         6         7
   @ 28,01 say [N§ÄC¢digoÄÄÄÄÄÄÄÄQuantidade Descri‡„oÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄVlr.Unit] color Cor(2)
