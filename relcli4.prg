 ...
 * Versao.......: 2.00
 * Identificacao: Relatorios de Clientes Por Vendedor
 * Prefixo......: SA
...
procedure RelCli4
   ...
   Msg(.f.)
   AtivaF4()
   Window(09,21,13,57," Clientes por Vendedor ")
   setcolor(Cor(11))
   @ 11,23 say "Vendedor:"
   while .t.
      cCodVen := space(02)
      setcolor(Cor(8)+","+Cor(9)+",,,"+Cor(8))
      @ 11,33 get cCodVen picture "@k 99" when Rodape("Esc-Encerra | F4-Vendedores") valid Busca(Zera(@cCodVen),"Vendedor",1,11,36,"Vendedor->Nome",{"Vendedor Nao Cadastrado"},.f.,.f.,.f.)
      setcursor(SC_NORMAL)
      read
      setcursor(SC_NONE)
      if lastkey() == K_ESC
      ...
      if !Clientes->(dbsetorder(8),dbseek(cCodVen))
   ...
   return
//*****************************************************************************
static procedure Imprima(cCodVen)
   local nVideo,lCabec := .t.,lTem := .f.,nTecla := 0,nQtd := 0
   private nPagina := 1

   Clientes->(dbsetorder(2),dbgotop())
   If Aviso_1( 09,,14,,[Aten‡„o!],[Imprimir Clientes por Vendedor?],{[  ^Sim  ],[  ^N„o  ]},1,.t.) == 1
      If Ver_Imp(@nVideo)
         T_IPorta := "USB"
         begin sequence
            Msg(.t.)
            ...
            Set Device to Print
            while Clientes->(!eof())
               if Clientes->CodVen == cCodVen
                  if lCabec
                     cabec(140,cEmpFantasia,{"Relacao de Clientes por Vendedor ","Vendedor: "+cCodVen+"-"+Vendedor->Nome})
                  ...
                  @ prow()+1,00 say "    Fone: "+transform(Clientes->TelCli1,"@r (999)9999-9999")+" / "+transform(Clientes->TelCli2,"@r (999)9999-9999")+space(27)+" Fax: "+transform(Clientes->FaxCli,"@kr (99)9999-9999")+" Celular: "+Clientes->CelCli
                  ...
                  @ prow()+1,00 say " Contato: "+Clientes->ConCli
